...
func (dq *DocumentQuery) QueryPosts() *PostQuery {
	query := &PostQuery{config: dq.config}
...
func (dq *DocumentQuery) WithPosts(opts ...func(*PostQuery)) *DocumentQuery {
	query := &PostQuery{config: dq.config}
...
//		GroupBy(document.FieldFilename).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (dq *DocumentQuery) sqlAll(ctx context.Context) ([]*Document, error) {
	...
	if query := dq.withPosts; query != nil {
		...
		ids := make(map[int]*Document, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(document.PostsPrimaryKey[0], fks...))
			},
