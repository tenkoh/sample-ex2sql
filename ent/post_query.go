...
func (pq *PostQuery) QueryDocument() *DocumentQuery {
	query := &DocumentQuery{config: pq.config}
...
func (pq *PostQuery) WithDocument(opts ...func(*DocumentQuery)) *PostQuery {
	query := &DocumentQuery{config: pq.config}
	...
	if query := pq.withDocument; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(post.DocumentPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, pq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "document": %w`, err)
		}
		query.Where(document.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "document" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Document = append(nodes[i].Edges.Document, n)
			}
		}
	}
